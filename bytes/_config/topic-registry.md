# Bytes - Topic Registry

> This registry maps topics to their byte folders and links them to
> existing dictionary categories where applicable. Start with core
> topics; grow organically as new topics are added.

---

## Spec References

| File                                                | Purpose                                      |
| --------------------------------------------------- | -------------------------------------------- |
| `bytes/_config/BYTES_KEYWORD_GENERATOR.md`          | Master keyword generation spec (v1.0)        |
| `.github/prompts/bytes-generate-keywords.prompt.md` | Prompt for topic/subtopic keyword processing |
| `bytes/_config/BYTES_PROMPT.md`                     | Master content generation spec (v1.0)        |

## Design Considerations

1. **New topic (no folder/index.md):** Use `bytes/_config/BYTES_KEYWORD_GENERATOR.md` v1.0 to generate keywords. Create folder + index.md. Group keywords into sub-topic files (5+ keywords per file). Generate content per `BYTES_PROMPT.md`.
2. **Brand-new topic (e.g., Angular):** Analyse where the topic belongs. Generate keywords via `bytes/_config/BYTES_KEYWORD_GENERATOR.md`. Create folder + files. Generate content.
3. **New subtopic (e.g., React Hooks, topic exists):** Create file in existing folder. Generate keywords via `bytes/_config/BYTES_KEYWORD_GENERATOR.md`. Generate content.
4. **Existing dictionary category (e.g., JVM, JCC):** Scan dictionary `index.md`. Analyse keywords. Map to byte sub-topic files. Generate content.

---

## Registry Format

| Topic        | Folder         | Dictionary Sources           | Status                                       |
| ------------ | -------------- | ---------------------------- | -------------------------------------------- |
| [Topic Name] | [folder-name/] | [CODE1, CODE2, ...] or "new" | planned / scaffolded / generating / complete |

---

## Active Topics

| Topic            | Folder            | Dictionary Sources | Status  | Description                                                                   |
| ---------------- | ----------------- | ------------------ | ------- | ----------------------------------------------------------------------------- |
| Java             | java/             | JVM, JLG           | planned | Core Java language, OOP, collections, modern Java features, JVM internals, GC |
| Java Concurrency | java-concurrency/ | JCC                | planned | Threading, synchronization, virtual threads, concurrent collections           |

---

## Sub-topic File Mapping

Each topic is split into sub-topic files. Each file MUST contain at
least 5 related keywords (no per-keyword files). Files are grouped by
relatedness and must be self-sufficient.

### Java (java/)

| File                               | Keywords (approximate)                                                                                                                                                 | Source IDs                          |
| ---------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------- |
| Java - Basics.md                   | Variables / Data Types, Operators / Control Flow, Classes / Objects, Inheritance, Interfaces, Access Modifiers, Enums                                                  | JLG-019 to JLG-027                  |
| Java - Collections.md              | Collections Framework, ArrayList / LinkedList, HashMap / TreeMap, HashSet, Queue / Deque, Iterator, Comparable / Comparator, equals / hashCode                         | JLG-029, JLG-077 / 078 / 080        |
| Java - Exceptions and IO.md        | Exception Hierarchy, Checked vs Unchecked, Try-with-Resources, Custom Exceptions, File IO, NIO, Serialization, Logging                                                 | JLG-030 / 031 / 040 / 070 / 076     |
| Java - Java 8 Features.md          | Lambdas, Functional Interfaces, Stream API, Optional, Method References, Default Methods, DateTime API, Collectors                                                     | JLG-035 to JLG-038, JLG-072/075/079 |
| Java - Java 11 to 17.md            | var, Text Blocks, Switch Expressions, Records, Sealed Classes, Pattern Matching instanceof, JPMS, HttpClient                                                           | JLG-032 / 039 / 071 / 082 / 083     |
| Java - Java 21 and Beyond.md       | Virtual Threads Patterns, Structured Concurrency, Scoped Values, Pattern Matching switch, Record Patterns, Sequenced Collections, String Templates, Foreign Memory API | JLG-049 / 097 / 098                 |
| Java - JVM Internals.md            | JVM Architecture, JVM / JRE / JDK, Bytecode, Class Loading, Stack / Heap, Metaspace, JIT (C1 / C2), Escape Analysis, GraalVM                                           | JVM-001 to JVM-057                  |
| Java - Garbage Collection.md       | GC Fundamentals, GC Roots, Generational GC, Serial / Parallel GC, G1GC, ZGC, Shenandoah, GC Tuning / Logs, Reference Types                                             | JVM-037 to JVM-048                  |
| Java - Diagnostics and Security.md | JFR, Thread Dumps, Heap Dumps, Performance Tuning, GC Selection Framework, Java Security, Version Migration                                                            | JVM-063 to JVM-067, JLG-015 / 016   |

### Java Concurrency (java-concurrency/)

| File                                         | Keywords (approximate)                                                                               | Source IDs         |
| -------------------------------------------- | ---------------------------------------------------------------------------------------------------- | ------------------ |
| Java Concurrency - Basics.md                 | Thread, Runnable, Callable, Future, Thread Lifecycle, start vs run, join, sleep, yield, interrupt    | JCC-001 to JCC-015 |
| Java Concurrency - Synchronization.md        | synchronized, volatile, Locks, ReentrantLock, ReadWriteLock, StampedLock, Conditions, Memory Model   | JCC-016 to JCC-035 |
| Java Concurrency - Executors.md              | ExecutorService, ThreadPoolExecutor, ScheduledExecutorService, ForkJoinPool, CompletionService       | JCC-036 to JCC-050 |
| Java Concurrency - Concurrent Collections.md | ConcurrentHashMap, CopyOnWriteArrayList, BlockingQueue, ConcurrentLinkedQueue, ConcurrentSkipListMap | JCC-051 to JCC-065 |
| Java Concurrency - Virtual Threads.md        | Virtual Threads, Carrier Threads, Structured Concurrency, Scoped Values, Pinning                     | JCC-066 to JCC-080 |

---

## Usage

To generate keywords for a topic, invoke:

```pwsh
pwsh -File bytes/_config/generate-keywords.ps1 -Topic Java -FromDictionary "JVM,JLG"
```

Or via Copilot:

```
@bytes-generate-keywords target: JLG
```

Both paths apply the v1.0 specification in
`bytes/_config/BYTES_KEYWORD_GENERATOR.md` exactly. This is
non-negotiable.
