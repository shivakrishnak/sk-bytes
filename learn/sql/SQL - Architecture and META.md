...
**TL;DR** - Sharding splits data across multiple database instances by a partition key; the critical architectural choice is whether routing lives in application code or in a proxy layer between app and databases.
...
- Skeema.io: "Tengo" - schema diff and migration for MySQL/PostgreSQL
...
**TL;DR** - GDPR's right-to-erasure requires the ability to delete all personal data for a given individual across every table, backup, replica, and downstream system - a problem SQL schemas are not designed for.
...
flowchart LR
    RC["READ COMMITTED\n(prevents: dirty read)"]
    RR_PG["REPEATABLE READ (PG)\n(prevents: +non-repeatable\n+phantom)\n(allows: write skew)"]
    RR_My["REPEATABLE READ (MySQL)\n(prevents: +non-repeatable\n+phantom via locks)\n(allows: write skew)"]
...
**TL;DR** - Relational algebra is the mathematical foundation of SQL: a set of operations (select, project, join, union) on relations that the query planner uses to transform your SQL into an optimal execution plan.
