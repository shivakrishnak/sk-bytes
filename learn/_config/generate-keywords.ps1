<#
.SYNOPSIS
    sk-learn - Keyword Generator & Folder Scaffolding
.DESCRIPTION
    Generates keyword lists for learn topics, groups them into
    sub-topic files, and creates folder/index/stub structure.

    Uses learn/_config/LEARN_KEYWORD_GENERATOR.md (Category Keyword
    Generator v1.0) as the master specification for all keyword
    generation. The prompt file at
    .github/prompts/learn-generate-keywords.prompt.md orchestrates
    this process for learn topics.

    Supports four flows:
    1. New topic from scratch (generates keywords via
       learn/_config/LEARN_KEYWORD_GENERATOR.md v1.0 spec)
    ...
    - For new topics without an index.md, use
      learn/_config/LEARN_KEYWORD_GENERATOR.md to generate a
      comprehensive keyword list, then apply folder/file rules
      and generate file content per LEARN_PROMPT.md v1.0.
    - For brand-new topics (e.g., Angular), analyse where the
      topic belongs, generate keywords via LEARN_KEYWORD_GENERATOR.md,
    ...
    - For new subtopics where the main topic already exists,
      ...
      LEARN_KEYWORD_GENERATOR.md, and generate content.
    - For existing dictionary categories (e.g., JVM, JCC),
      scan the dictionary index.md, analyse keywords, map to
      learn sub-topic files, and generate content.

    ALWAYS use pwsh (PowerShell 7+):
      pwsh -ExecutionPolicy Bypass -File learn/_config/generate-keywords.ps1

    REFERENCES:
    - learn/_config/LEARN_KEYWORD_GENERATOR.md: Master keyword spec
    - .github/prompts/learn-generate-keywords.prompt.md: Prompt file
    - learn/_config/LEARN_PROMPT.md: Content generation spec

.PARAMETER Topic
    Topic name (e.g., "Java", "Angular", "Kubernetes")
.PARAMETER FromDictionary
    Dictionary category code(s) to source keywords from
    (e.g., "JLG", "JVM,JLG"). Optional. If sk-learn is a standalone
...
$LearnRoot     = Join-Path $WorkspaceRoot "learn"
...
function Get-TopicPath {
    param([string]$TopicName)
    return Join-Path $LearnRoot (Get-TopicFolder $TopicName)
}
...
nav_order: 1
has_children: true
permalink: /learn/$TopicFolder/
description: Bite-sized study units for $TopicName
keywords_count: 0
files_count: 0
---
...
> Fill via @learn-generate-entries or /learn agent per LEARN_PROMPT.md v1.0.
...
# Workflows
Write-Host ""
Write-Host "=== sk-learn Keyword Generator ==="
Write-Host "Spec: learn/_config/LEARN_KEYWORD_GENERATOR.md (v1.0)"
Write-Host ""
...
if ($Subtopic) {
    ...
    Write-Host ""
    Write-Host "Done. Next: invoke @learn-generate-keywords to expand keyword"
    Write-Host "       coverage to 7 mastery levels per LEARN_KEYWORD_GENERATOR.md."
    exit 0
}
...
if (-not (Test-Path $indexPath)) {
    $desc = "Bite-sized study units for $Topic."
...
Write-Host ""
Write-Host "Next step: invoke @learn-generate-keywords with:"
Write-Host "  target: $Topic"
if ($FromDictionary) {
    Write-Host "  from:   $FromDictionary"
}
Write-Host ""
Write-Host "The prompt will apply LEARN_KEYWORD_GENERATOR.md v1.0:"
